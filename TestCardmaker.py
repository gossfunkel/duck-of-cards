	...
	def generateTerrain(self):
		...
		for x in range(width):
			...
			for y in range(height):
				...
				#y -= 0.5
				tileInd += 1
				#tile = self.render.attachNewNode()
				tileScaleFactor = sqrt(2) # pythagoras; turning squares sideways makes triangles maybe?
				...
				tile = self.render.attachNewNode(card_maker.generate())#"tileGRASS"+str(x)+":"+str(y)+"-"+str(tileInd)
				tile.setPos(x-y,x+y,0.)
				tile.setHpr(0., -90., 45.)
				tile.setTexture(groundTile, 1)
